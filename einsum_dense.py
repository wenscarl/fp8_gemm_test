...
def update_scale(x, quantized_dtype, scale_var, amax_history):
  ...
  # amax_his_tsr = tf.convert_to_tensor(amax_history)
...
def qdq_and_update(x, dtype, scale_var, amax_history):
  ...
  # amax_history_updated = None
  # update_scale_old(x, dtype, scale_var)
  return qx
...
class EinsumDense(Layer):
    ...
    **Applying a dense layer to a sequence using ellipses**
    ...
    def in_qdq(self, input):
      ...
      def grad(in_grad):
        in_grad_ret = qdq_and_update(in_grad, FAKE_E5M2, self.input_grad_scale, self.input_grad_amax_history)
        return in_grad_ret
    ...
    def kernel_qdq(self, kernel):
      ...
      qkernel = qdq_and_update(kernel, FAKE_E4M3, self.kernel_scale, self.kernel_amax_history)
