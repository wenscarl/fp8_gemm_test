...
def quantize(x, quantized_dtype, scale):
    ...
    #  print("qqqqqq", x.dtype, scale.dtype)
    scaled_x = tf.clip_by_value(x / tf.cast(scale, x.dtype), -dtype_max, dtype_max)
...
class DotProductAttention(tf.keras.Model):
    ...
    def call(
        ...
        if use_fp8_bmm:
            bmm1 = (
                tf.matmul(self.in_qdq(query), self.kernel_qdq(key)) / self.norm_factor
            )
            bmm1 = self.out_qdq(bmm1)
        else:
        ...
        if use_fp8_bmm:
            xx = self.in_qdq(attention_probs)
            yy = self.kernel_qdq(value)
            context = tf.matmul(xx, yy)
            context = self.out_qdq(context)
        else:
...
# Layer configuration
hidden_size = 512#4096 * model_size_scale
sequence_length = 256#2048
batch_size = 4
ffn_hidden_size = 128#16384 * model_size_scale
